...
```bash
mleip-lambda-example$ sam build
```
...
```bash
mleip-lambda-example$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
mleip-lambda-example$ sam local start-api
mleip-lambda-example$ curl http://localhost:3000/classify_digit
```
...
```bash
mleip-lambda-example$ sam logs -n InferenceFunction --stack-name mleip-lambda-example --tail
```
...
```bash
aws cloudformation delete-stack --stack-name mleip-lambda-example
```
